...
def colicion(d_tablero,pos_xy):
    ...
    if(tarjeta2.Tarjeta.cantidad_tarjetas_visibles_no_descubiertas(lista_tarjetas) < 2):
...
def cantidad_tarjetas_visibles_no_descubiertas2(lista_tarjetas):
    cantidad = 0
...
def update(d_tablero):
    ...
    if(d_tablero["tiempo_ultimo_destape"] > 0):
        if(tarjeta2.Tarjeta.match(d_tablero["l_tarjetas"])):
             d_tablero["tiempo_ultimo_destape"] = 0
